<?php
	 ...
	 * listing a collection of LemonloversAssessmentQuestions objects.  It includes
	 ...
	 * 
	 */
	class LemonloversAssessmentQuestionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this LemonloversAssessmentQuestions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the LemonloversAssessmentQuestions object, or it can be any QQNode extending from QQN::LemonloversAssessmentQuestions().
		 * 
		 ...
		 * sorting using two fields from the LemonloversAssessmentQuestions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from LemonloversAssessmentQuestions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = LemonloversAssessmentQuestions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from LemonloversAssessmentQuestions, given the clauses above
			$this->DataSource = LemonloversAssessmentQuestions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a LemonloversAssessmentQuestions-based QQNode.
		 * It will also verify that it is a proper LemonloversAssessmentQuestions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'lemonlovers_assessment_questions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "lemonlovers_assessment_questions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::LemonloversAssessmentQuestions()->Id;
				case 'Count': return QQN::LemonloversAssessmentQuestions()->Count;
				case 'Text': return QQN::LemonloversAssessmentQuestions()->Text;
				case 'LemonTypeId': return QQN::LemonloversAssessmentQuestions()->LemonTypeId;
				case 'Inverse': return QQN::LemonloversAssessmentQuestions()->Inverse;
				default: throw new QCallerException('Simple Property not found in LemonloversAssessmentQuestionsDataGrid content: ' . $mixContent);
