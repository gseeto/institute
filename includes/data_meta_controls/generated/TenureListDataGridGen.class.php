<?php
	 ...
	 * listing a collection of TenureList objects.  It includes
	 ...
	 * 
	 */
	class TenureListDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TenureList datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TenureList object, or it can be any QQNode extending from QQN::TenureList().
		 * 
		 ...
		 * sorting using two fields from the TenureList object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TenureList
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TenureList::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TenureList, given the clauses above
			$this->DataSource = TenureList::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TenureList-based QQNode.
		 * It will also verify that it is a proper TenureList-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'tenure_list') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tenure_list".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::TenureList()->Id;
				case 'Range': return QQN::TenureList()->Range;
				default: throw new QCallerException('Simple Property not found in TenureListDataGrid content: ' . $mixContent);
