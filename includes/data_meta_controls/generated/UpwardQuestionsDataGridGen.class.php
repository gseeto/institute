<?php
	 ...
	 * listing a collection of UpwardQuestions objects.  It includes
	 ...
	 * 
	 */
	class UpwardQuestionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this UpwardQuestions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the UpwardQuestions object, or it can be any QQNode extending from QQN::UpwardQuestions().
		 * 
		 ...
		 * sorting using two fields from the UpwardQuestions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from UpwardQuestions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = UpwardQuestions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from UpwardQuestions, given the clauses above
			$this->DataSource = UpwardQuestions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a UpwardQuestions-based QQNode.
		 * It will also verify that it is a proper UpwardQuestions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'upward_questions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "upward_questions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::UpwardQuestions()->Id;
				case 'Count': return QQN::UpwardQuestions()->Count;
				case 'Text': return QQN::UpwardQuestions()->Text;
				case 'CategoryId': return QQN::UpwardQuestions()->CategoryId;
				default: throw new QCallerException('Simple Property not found in UpwardQuestionsDataGrid content: ' . $mixContent);
