<?php
	 ...
	 * listing a collection of Role objects.  It includes
	 ...
	 * 
	 */
	class RoleDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Role datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Role object, or it can be any QQNode extending from QQN::Role().
		 * 
		 ...
		 * sorting using two fields from the Role object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Role
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Role::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Role, given the clauses above
			$this->DataSource = Role::QueryArray($objCondition, $objClauses);
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'Id': return QQN::Role()->Id;
				case 'Name': return QQN::Role()->Name;
				default: throw new QCallerException('Simple Property not found in RoleDataGrid content: ' . $mixContent);
