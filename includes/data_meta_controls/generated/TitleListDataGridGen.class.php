<?php
	 ...
	 * listing a collection of TitleList objects.  It includes
	 ...
	 * 
	 */
	class TitleListDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TitleList datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TitleList object, or it can be any QQNode extending from QQN::TitleList().
		 * 
		 ...
		 * sorting using two fields from the TitleList object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TitleList
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TitleList::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TitleList, given the clauses above
			$this->DataSource = TitleList::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TitleList-based QQNode.
		 * It will also verify that it is a proper TitleList-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'title_list') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "title_list".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::TitleList()->Id;
				case 'Name': return QQN::TitleList()->Name;
				default: throw new QCallerException('Simple Property not found in TitleListDataGrid content: ' . $mixContent);
