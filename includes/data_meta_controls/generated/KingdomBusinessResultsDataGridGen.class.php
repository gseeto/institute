<?php
	 ...
	 * listing a collection of KingdomBusinessResults objects.  It includes
	 ...
	 * 
	 */
	class KingdomBusinessResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this KingdomBusinessResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the KingdomBusinessResults object, or it can be any QQNode extending from QQN::KingdomBusinessResults().
		 * 
		 ...
		 * sorting using two fields from the KingdomBusinessResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from KingdomBusinessResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = KingdomBusinessResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from KingdomBusinessResults, given the clauses above
			$this->DataSource = KingdomBusinessResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a KingdomBusinessResults-based QQNode.
		 * It will also verify that it is a proper KingdomBusinessResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'kingdom_business_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "kingdom_business_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::KingdomBusinessResults()->Id;
				case 'QuestionId': return QQN::KingdomBusinessResults()->QuestionId;
				case 'Question': return QQN::KingdomBusinessResults()->Question;
				case 'AssessmentId': return QQN::KingdomBusinessResults()->AssessmentId;
				case 'Assessment': return QQN::KingdomBusinessResults()->Assessment;
				case 'Performance': return QQN::KingdomBusinessResults()->Performance;
				case 'Importance': return QQN::KingdomBusinessResults()->Importance;
				default: throw new QCallerException('Simple Property not found in KingdomBusinessResultsDataGrid content: ' . $mixContent);
