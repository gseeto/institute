<?php
	 ...
	 * listing a collection of BusinessChecklist objects.  It includes
	 ...
	 * 
	 */
	class BusinessChecklistDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this BusinessChecklist datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the BusinessChecklist object, or it can be any QQNode extending from QQN::BusinessChecklist().
		 * 
		 ...
		 * sorting using two fields from the BusinessChecklist object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from BusinessChecklist
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = BusinessChecklist::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from BusinessChecklist, given the clauses above
			$this->DataSource = BusinessChecklist::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a BusinessChecklist-based QQNode.
		 * It will also verify that it is a proper BusinessChecklist-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'business_checklist') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "business_checklist".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::BusinessChecklist()->Id;
				case 'CompanyId': return QQN::BusinessChecklist()->CompanyId;
				case 'Company': return QQN::BusinessChecklist()->Company;
				default: throw new QCallerException('Simple Property not found in BusinessChecklistDataGrid content: ' . $mixContent);
