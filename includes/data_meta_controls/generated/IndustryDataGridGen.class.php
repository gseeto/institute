<?php
	 ...
	 * listing a collection of Industry objects.  It includes
	 ...
	 * 
	 */
	class IndustryDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Industry datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Industry object, or it can be any QQNode extending from QQN::Industry().
		 * 
		 ...
		 * sorting using two fields from the Industry object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Industry
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Industry::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Industry, given the clauses above
			$this->DataSource = Industry::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Industry-based QQNode.
		 * It will also verify that it is a proper Industry-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'industry') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "industry".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Industry()->Id;
				case 'Value': return QQN::Industry()->Value;
				default: throw new QCallerException('Simple Property not found in IndustryDataGrid content: ' . $mixContent);
