<?php
	 ...
	 * listing a collection of TimeResults objects.  It includes
	 ...
	 * 
	 */
	class TimeResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TimeResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TimeResults object, or it can be any QQNode extending from QQN::TimeResults().
		 * 
		 ...
		 * sorting using two fields from the TimeResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TimeResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TimeResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TimeResults, given the clauses above
			$this->DataSource = TimeResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TimeResults-based QQNode.
		 * It will also verify that it is a proper TimeResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'time_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "time_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::TimeResults()->Id;
				case 'AssessmentId': return QQN::TimeResults()->AssessmentId;
				case 'Assessment': return QQN::TimeResults()->Assessment;
				case 'Time': return QQN::TimeResults()->Time;
				case 'Activity': return QQN::TimeResults()->Activity;
				case 'Career': return QQN::TimeResults()->Career;
				case 'Calling': return QQN::TimeResults()->Calling;
				case 'Community': return QQN::TimeResults()->Community;
				case 'Creativity': return QQN::TimeResults()->Creativity;
				case 'Margin': return QQN::TimeResults()->Margin;
				default: throw new QCallerException('Simple Property not found in TimeResultsDataGrid content: ' . $mixContent);
