<?php
	 ...
	 * listing a collection of User objects.  It includes
	 ...
	 * 
	 */
	class UserDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this User datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the User object, or it can be any QQNode extending from QQN::User().
		 * 
		 ...
		 * sorting using two fields from the User object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from User
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = User::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from User, given the clauses above
			$this->DataSource = User::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a User-based QQNode.
		 * It will also verify that it is a proper User-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'user') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "user".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::User()->Id;
				case 'LoginId': return QQN::User()->LoginId;
				case 'Login': return QQN::User()->Login;
				case 'FirstName': return QQN::User()->FirstName;
