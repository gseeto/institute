<?php
	 ...
	 * listing a collection of IntegrationQuestions objects.  It includes
	 ...
	 * 
	 */
	class IntegrationQuestionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this IntegrationQuestions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the IntegrationQuestions object, or it can be any QQNode extending from QQN::IntegrationQuestions().
		 * 
		 ...
		 * sorting using two fields from the IntegrationQuestions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from IntegrationQuestions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = IntegrationQuestions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from IntegrationQuestions, given the clauses above
			$this->DataSource = IntegrationQuestions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a IntegrationQuestions-based QQNode.
		 * It will also verify that it is a proper IntegrationQuestions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'integration_questions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "integration_questions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::IntegrationQuestions()->Id;
				case 'Count': return QQN::IntegrationQuestions()->Count;
				case 'Text': return QQN::IntegrationQuestions()->Text;
				case 'CategoryId': return QQN::IntegrationQuestions()->CategoryId;
				case 'Invert': return QQN::IntegrationQuestions()->Invert;
				default: throw new QCallerException('Simple Property not found in IntegrationQuestionsDataGrid content: ' . $mixContent);
