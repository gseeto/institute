<?php
	 ...
	 * listing a collection of SeasonalResults objects.  It includes
	 ...
	 * 
	 */
	class SeasonalResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this SeasonalResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the SeasonalResults object, or it can be any QQNode extending from QQN::SeasonalResults().
		 * 
		 ...
		 * sorting using two fields from the SeasonalResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from SeasonalResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = SeasonalResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from SeasonalResults, given the clauses above
			$this->DataSource = SeasonalResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a SeasonalResults-based QQNode.
		 * It will also verify that it is a proper SeasonalResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'seasonal_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "seasonal_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::SeasonalResults()->Id;
				case 'QuestionId': return QQN::SeasonalResults()->QuestionId;
				case 'Question': return QQN::SeasonalResults()->Question;
				case 'AssessmentId': return QQN::SeasonalResults()->AssessmentId;
				case 'Assessment': return QQN::SeasonalResults()->Assessment;
				case 'Result': return QQN::SeasonalResults()->Result;
				default: throw new QCallerException('Simple Property not found in SeasonalResultsDataGrid content: ' . $mixContent);
