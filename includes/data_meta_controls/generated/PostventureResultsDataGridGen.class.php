<?php
	 ...
	 * listing a collection of PostventureResults objects.  It includes
	 ...
	 * 
	 */
	class PostventureResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PostventureResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PostventureResults object, or it can be any QQNode extending from QQN::PostventureResults().
		 * 
		 ...
		 * sorting using two fields from the PostventureResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PostventureResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PostventureResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from PostventureResults, given the clauses above
			$this->DataSource = PostventureResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PostventureResults-based QQNode.
		 * It will also verify that it is a proper PostventureResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'postventure_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "postventure_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PostventureResults()->Id;
				case 'QuestionId': return QQN::PostventureResults()->QuestionId;
				case 'Question': return QQN::PostventureResults()->Question;
				case 'AssessmentId': return QQN::PostventureResults()->AssessmentId;
				case 'Assessment': return QQN::PostventureResults()->Assessment;
				case 'Performance': return QQN::PostventureResults()->Performance;
				case 'Importance': return QQN::PostventureResults()->Importance;
				default: throw new QCallerException('Simple Property not found in PostventureResultsDataGrid content: ' . $mixContent);
