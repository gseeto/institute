<?php
	 ...
	 * listing a collection of PartneringAwarenessQuestions objects.  It includes
	 ...
	 * 
	 */
	class PartneringAwarenessQuestionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PartneringAwarenessQuestions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PartneringAwarenessQuestions object, or it can be any QQNode extending from QQN::PartneringAwarenessQuestions().
		 * 
		 ...
		 * sorting using two fields from the PartneringAwarenessQuestions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PartneringAwarenessQuestions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PartneringAwarenessQuestions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from PartneringAwarenessQuestions, given the clauses above
			$this->DataSource = PartneringAwarenessQuestions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PartneringAwarenessQuestions-based QQNode.
		 * It will also verify that it is a proper PartneringAwarenessQuestions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'partnering_awareness_questions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "partnering_awareness_questions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PartneringAwarenessQuestions()->Id;
				case 'Count': return QQN::PartneringAwarenessQuestions()->Count;
				case 'Text': return QQN::PartneringAwarenessQuestions()->Text;
				case 'CategoryId': return QQN::PartneringAwarenessQuestions()->CategoryId;
				default: throw new QCallerException('Simple Property not found in PartneringAwarenessQuestionsDataGrid content: ' . $mixContent);
