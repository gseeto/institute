<?php
	 ...
	 * listing a collection of Strategy objects.  It includes
	 ...
	 * 
	 */
	class StrategyDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Strategy datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Strategy object, or it can be any QQNode extending from QQN::Strategy().
		 * 
		 ...
		 * sorting using two fields from the Strategy object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Strategy
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Strategy::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Strategy, given the clauses above
			$this->DataSource = Strategy::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Strategy-based QQNode.
		 * It will also verify that it is a proper Strategy-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'strategy') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "strategy".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Strategy()->Id;
				case 'Strategy': return QQN::Strategy()->Strategy;
				case 'Priority': return QQN::Strategy()->Priority;
				case 'Count': return QQN::Strategy()->Count;
				case 'ScorecardId': return QQN::Strategy()->ScorecardId;
				case 'Scorecard': return QQN::Strategy()->Scorecard;
				case 'CategoryTypeId': return QQN::Strategy()->CategoryTypeId;
				case 'ModifiedBy': return QQN::Strategy()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Strategy()->ModifiedByObject;
				case 'Modified': return QQN::Strategy()->Modified;
				default: throw new QCallerException('Simple Property not found in StrategyDataGrid content: ' . $mixContent);
