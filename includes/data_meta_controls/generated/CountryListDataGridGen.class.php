<?php
	 ...
	 * listing a collection of CountryList objects.  It includes
	 ...
	 * 
	 */
	class CountryListDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CountryList datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CountryList object, or it can be any QQNode extending from QQN::CountryList().
		 * 
		 ...
		 * sorting using two fields from the CountryList object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CountryList
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CountryList::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CountryList, given the clauses above
			$this->DataSource = CountryList::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CountryList-based QQNode.
		 * It will also verify that it is a proper CountryList-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'country_list') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "country_list".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CountryList()->Id;
				case 'Name': return QQN::CountryList()->Name;
				default: throw new QCallerException('Simple Property not found in CountryListDataGrid content: ' . $mixContent);
