<?php
	 ...
	 * listing a collection of PostventureQuestions objects.  It includes
	 ...
	 * 
	 */
	class PostventureQuestionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PostventureQuestions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PostventureQuestions object, or it can be any QQNode extending from QQN::PostventureQuestions().
		 * 
		 ...
		 * sorting using two fields from the PostventureQuestions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PostventureQuestions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PostventureQuestions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from PostventureQuestions, given the clauses above
			$this->DataSource = PostventureQuestions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PostventureQuestions-based QQNode.
		 * It will also verify that it is a proper PostventureQuestions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'postventure_questions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "postventure_questions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PostventureQuestions()->Id;
				case 'Count': return QQN::PostventureQuestions()->Count;
				case 'Text': return QQN::PostventureQuestions()->Text;
				case 'CategoryId': return QQN::PostventureQuestions()->CategoryId;
				default: throw new QCallerException('Simple Property not found in PostventureQuestionsDataGrid content: ' . $mixContent);
