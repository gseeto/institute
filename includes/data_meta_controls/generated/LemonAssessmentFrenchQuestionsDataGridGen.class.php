<?php
	 ...
	 * listing a collection of LemonAssessmentFrenchQuestions objects.  It includes
	 ...
	 * 
	 */
	class LemonAssessmentFrenchQuestionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this LemonAssessmentFrenchQuestions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the LemonAssessmentFrenchQuestions object, or it can be any QQNode extending from QQN::LemonAssessmentFrenchQuestions().
		 * 
		 ...
		 * sorting using two fields from the LemonAssessmentFrenchQuestions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from LemonAssessmentFrenchQuestions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = LemonAssessmentFrenchQuestions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from LemonAssessmentFrenchQuestions, given the clauses above
			$this->DataSource = LemonAssessmentFrenchQuestions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a LemonAssessmentFrenchQuestions-based QQNode.
		 * It will also verify that it is a proper LemonAssessmentFrenchQuestions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'lemon_assessment_french_questions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "lemon_assessment_french_questions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Count': return QQN::LemonAssessmentFrenchQuestions()->Count;
				case 'Text': return QQN::LemonAssessmentFrenchQuestions()->Text;
				case 'LemonTypeId': return QQN::LemonAssessmentFrenchQuestions()->LemonTypeId;
				case 'Inverse': return QQN::LemonAssessmentFrenchQuestions()->Inverse;
				case 'Id': return QQN::LemonAssessmentFrenchQuestions()->Id;
				case 'IdObject': return QQN::LemonAssessmentFrenchQuestions()->IdObject;
				default: throw new QCallerException('Simple Property not found in LemonAssessmentFrenchQuestionsDataGrid content: ' . $mixContent);
