<?php
	 ...
	 * listing a collection of IntegrationAssessment objects.  It includes
	 ...
	 * 
	 */
	class IntegrationAssessmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this IntegrationAssessment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the IntegrationAssessment object, or it can be any QQNode extending from QQN::IntegrationAssessment().
		 * 
		 ...
		 * sorting using two fields from the IntegrationAssessment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from IntegrationAssessment
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = IntegrationAssessment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from IntegrationAssessment, given the clauses above
			$this->DataSource = IntegrationAssessment::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a IntegrationAssessment-based QQNode.
		 * It will also verify that it is a proper IntegrationAssessment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'integration_assessment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "integration_assessment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::IntegrationAssessment()->Id;
				case 'ResourceStatusId': return QQN::IntegrationAssessment()->ResourceStatusId;
				case 'ResourceStatus': return QQN::IntegrationAssessment()->ResourceStatus;
				case 'ResourceId': return QQN::IntegrationAssessment()->ResourceId;
				case 'Resource': return QQN::IntegrationAssessment()->Resource;
				case 'UserId': return QQN::IntegrationAssessment()->UserId;
				case 'User': return QQN::IntegrationAssessment()->User;
				case 'GroupId': return QQN::IntegrationAssessment()->GroupId;
				case 'Group': return QQN::IntegrationAssessment()->Group;
				default: throw new QCallerException('Simple Property not found in IntegrationAssessmentDataGrid content: ' . $mixContent);
