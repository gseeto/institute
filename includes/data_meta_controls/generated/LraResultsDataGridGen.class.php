<?php
	 ...
	 * listing a collection of LraResults objects.  It includes
	 ...
	 * 
	 */
	class LraResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this LraResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the LraResults object, or it can be any QQNode extending from QQN::LraResults().
		 * 
		 ...
		 * sorting using two fields from the LraResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from LraResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = LraResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from LraResults, given the clauses above
			$this->DataSource = LraResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a LraResults-based QQNode.
		 * It will also verify that it is a proper LraResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'lra_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "lra_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::LraResults()->Id;
				case 'QuestionId': return QQN::LraResults()->QuestionId;
				case 'Question': return QQN::LraResults()->Question;
				case 'AssessmentId': return QQN::LraResults()->AssessmentId;
				case 'Assessment': return QQN::LraResults()->Assessment;
				case 'Head': return QQN::LraResults()->Head;
				case 'Hands': return QQN::LraResults()->Hands;
				case 'Heart': return QQN::LraResults()->Heart;
				default: throw new QCallerException('Simple Property not found in LraResultsDataGrid content: ' . $mixContent);
