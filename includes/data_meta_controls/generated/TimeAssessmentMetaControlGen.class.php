<?php
	 ...
	 * of the TimeAssessment class.  This code-generated class
	 * contains all the basic elements to help a QPanel or QForm display an HTML form that can
	 * manipulate a single TimeAssessment object.
	 *
	 * To take advantage of some (or all) of these control objects, you
	 * must create a new QForm or QPanel which instantiates a TimeAssessmentMetaControl
	 * class.
	 *
	 ...
	 * property-read TimeAssessment $TimeAssessment the actual TimeAssessment data class being edited
	...
	class TimeAssessmentMetaControlGen extends QBaseClass {
		// General Variables
		/**
		 * @var TimeAssessment objTimeAssessment
		 * @access protected
		 */
		...
		// Controls that allow the editing of TimeAssessment's individual data fields
        /**
         * @var QLabel lblId;
         * @access protected
         */
		...
		// Controls that allow the viewing of TimeAssessment's individual data fields
        /**
		 ...
		 * TimeAssessmentMetaControl to edit a single TimeAssessment object within the
		 * QPanel or QForm.
		 *
		 * This constructor takes in a single TimeAssessment object, while any of the static
		 * create methods below can be used to construct based off of individual PK ID(s).
		 *
		 * @param mixed $objParentObject QForm or QPanel which will be using this TimeAssessmentMetaControl
		 * @param TimeAssessment $objTimeAssessment new or existing TimeAssessment object
		 */
		 public function __construct($objParentObject, TimeAssessment $objTimeAssessment) {
			// Setup Parent Object (e.g. QForm or QPanel which will be using this TimeAssessmentMetaControl)
			$this->objParentObject = $objParentObject;

			// Setup linked TimeAssessment object
			$this->objTimeAssessment = $objTimeAssessment;

			// Figure out if we're Editing or Creating New
			if ($this->objTimeAssessment->__Restored) {
		 ...
		 * @param QMetaControlCreateType $intCreateType rules governing TimeAssessment object creation - defaults to CreateOrEdit
 		 * @return TimeAssessmentMetaControl
		 */
		public static function Create($objParentObject, $intId = null, $intCreateType = QMetaControlCreateType::CreateOrEdit) {
			// Attempt to Load from PK Arguments
			if (strlen($intId)) {
				$objTimeAssessment = TimeAssessment::Load($intId);

				// TimeAssessment was found -- return it!
				if ($objTimeAssessment)
					return new TimeAssessmentMetaControl($objParentObject, $objTimeAssessment);

				// If CreateOnRecordNotFound not specified, throw an exception
				else if ($intCreateType != QMetaControlCreateType::CreateOnRecordNotFound)
					throw new QCallerException('Could not find a TimeAssessment object with PK arguments: ' . $intId);
			...
			return new TimeAssessmentMetaControl($objParentObject, new TimeAssessment());
		}
		...
		public static function CreateFromPathInfo($objParentObject, $intCreateType = QMetaControlCreateType::CreateOrEdit) {
			...
			return TimeAssessmentMetaControl::Create($objParentObject, $intId, $intCreateType);
		}
		...
		public function lblId_Create($strControlId = null) {
			...
			if ($this->blnEditMode)
				$this->lblId->Text = $this->objTimeAssessment->Id;
			else
				$this->lblId->Text = 'N/A';
			return $this->lblId;
		}
		...
		public function lstResourceStatus_Create($strControlId = null, QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			while ($objResourceStatus = ResourceStatus::InstantiateCursor($objResourceStatusCursor)) {
				...
				if (($this->objTimeAssessment->ResourceStatus) && ($this->objTimeAssessment->ResourceStatus->Id == $objResourceStatus->Id))
		...
		public function lblResourceStatusId_Create($strControlId = null) {
			...
			$this->lblResourceStatusId->Text = ($this->objTimeAssessment->ResourceStatus) ? $this->objTimeAssessment->ResourceStatus->__toString() : null;
		...
		public function lstResource_Create($strControlId = null, QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			while ($objResource = Resource::InstantiateCursor($objResourceCursor)) {
				...
				if (($this->objTimeAssessment->Resource) && ($this->objTimeAssessment->Resource->Id == $objResource->Id))
					$objListItem->Selected = true;
		...
		public function lblResourceId_Create($strControlId = null) {
			...
			$this->lblResourceId->Text = ($this->objTimeAssessment->Resource) ? $this->objTimeAssessment->Resource->__toString() : null;
		...
		public function lstUser_Create($strControlId = null, QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			while ($objUser = User::InstantiateCursor($objUserCursor)) {
				...
				if (($this->objTimeAssessment->User) && ($this->objTimeAssessment->User->Id == $objUser->Id))
					$objListItem->Selected = true;
		...
		public function lblUserId_Create($strControlId = null) {
			...
			$this->lblUserId->Text = ($this->objTimeAssessment->User) ? $this->objTimeAssessment->User->__toString() : null;
		...
		public function lstGroup_Create($strControlId = null, QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			while ($objGroup = GroupAssessmentList::InstantiateCursor($objGroupCursor)) {
				...
				if (($this->objTimeAssessment->Group) && ($this->objTimeAssessment->Group->Id == $objGroup->Id))
					$objListItem->Selected = true;
		...
		public function lblGroupId_Create($strControlId = null) {
			...
			$this->lblGroupId->Text = ($this->objTimeAssessment->Group) ? $this->objTimeAssessment->Group->__toString() : null;
		...
		/**
		 * Refresh this MetaControl with Data from the local TimeAssessment object.
		 * @param boolean $blnReload reload TimeAssessment from the database
		 * @return void
		 */
		public function Refresh($blnReload = false) {
			if ($blnReload)
				$this->objTimeAssessment->Reload();

			if ($this->lblId) if ($this->blnEditMode) $this->lblId->Text = $this->objTimeAssessment->Id;
			...
			if ($this->lblResourceStatusId) $this->lblResourceStatusId->Text = ($this->objTimeAssessment->ResourceStatus) ? $this->objTimeAssessment->ResourceStatus->__toString() : null;
			...
			if ($this->lblResourceId) $this->lblResourceId->Text = ($this->objTimeAssessment->Resource) ? $this->objTimeAssessment->Resource->__toString() : null;
			...
			if ($this->lblUserId) $this->lblUserId->Text = ($this->objTimeAssessment->User) ? $this->objTimeAssessment->User->__toString() : null;
			...
			if ($this->lblGroupId) $this->lblGroupId->Text = ($this->objTimeAssessment->Group) ? $this->objTimeAssessment->Group->__toString() : null;
		...
		///////////////////////////////////////////////
		// PUBLIC TIMEASSESSMENT OBJECT MANIPULATORS
		///////////////////////////////////////////////

		/**
		 * This will save this object's TimeAssessment instance,
		 * updating only the fields which have had a control created for it.
		 */
		public function SaveTimeAssessment() {
			try {
				// Update any fields for controls that have been created
				if ($this->lstResourceStatus) $this->objTimeAssessment->ResourceStatusId = $this->lstResourceStatus->SelectedValue;
				if ($this->lstResource) $this->objTimeAssessment->ResourceId = $this->lstResource->SelectedValue;
				if ($this->lstUser) $this->objTimeAssessment->UserId = $this->lstUser->SelectedValue;
				if ($this->lstGroup) $this->objTimeAssessment->GroupId = $this->lstGroup->SelectedValue;

				// Update any UniqueReverseReferences (if any) for controls that have been created for it

				// Save the TimeAssessment object
		...
		/**
		 * This will DELETE this object's TimeAssessment instance from the database.
		 * It will also unassociate itself from any ManyToManyReferences.
		 */
		public function DeleteTimeAssessment() {
			$this->objTimeAssessment->Delete();
		}		
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'TimeAssessment': return $this->objTimeAssessment;
				case 'TitleVerb': return $this->strTitleVerb;
				case 'EditMode': return $this->blnEditMode;

				// Controls that point to TimeAssessment fields -- will be created dynamically if not yet created
				case 'IdControl':
		...
		public function __set($strName, $mixValue) {
			try {
				switch ($strName) {
					// Controls that point to TimeAssessment fields
					case 'IdControl':
