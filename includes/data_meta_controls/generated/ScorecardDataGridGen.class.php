<?php
	 ...
	 * listing a collection of Scorecard objects.  It includes
	 ...
	 * 
	 */
	class ScorecardDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Scorecard datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Scorecard object, or it can be any QQNode extending from QQN::Scorecard().
		 * 
		 ...
		 * sorting using two fields from the Scorecard object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Scorecard
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Scorecard::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Scorecard, given the clauses above
			$this->DataSource = Scorecard::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Scorecard-based QQNode.
		 * It will also verify that it is a proper Scorecard-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'scorecard') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "scorecard".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Scorecard()->Id;
				case 'CompanyId': return QQN::Scorecard()->CompanyId;
				case 'Company': return QQN::Scorecard()->Company;
				case 'Name': return QQN::Scorecard()->Name;
				case 'ResourceId': return QQN::Scorecard()->ResourceId;
				case 'Resource': return QQN::Scorecard()->Resource;
				default: throw new QCallerException('Simple Property not found in ScorecardDataGrid content: ' . $mixContent);
