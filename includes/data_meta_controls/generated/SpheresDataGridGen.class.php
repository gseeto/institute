<?php
	 ...
	 * listing a collection of Spheres objects.  It includes
	 ...
	 * 
	 */
	class SpheresDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Spheres datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Spheres object, or it can be any QQNode extending from QQN::Spheres().
		 * 
		 ...
		 * sorting using two fields from the Spheres object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Spheres
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Spheres::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Spheres, given the clauses above
			$this->DataSource = Spheres::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Spheres-based QQNode.
		 * It will also verify that it is a proper Spheres-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'spheres') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "spheres".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Spheres()->Id;
				case 'Sphere': return QQN::Spheres()->Sphere;
				default: throw new QCallerException('Simple Property not found in SpheresDataGrid content: ' . $mixContent);
