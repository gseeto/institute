<?php
	 ...
	 * listing a collection of Resource objects.  It includes
	 ...
	 * 
	 */
	class ResourceDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Resource datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Resource object, or it can be any QQNode extending from QQN::Resource().
		 * 
		 ...
		 * sorting using two fields from the Resource object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Resource
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Resource::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Resource, given the clauses above
			$this->DataSource = Resource::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Resource-based QQNode.
		 * It will also verify that it is a proper Resource-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'resource') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "resource".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Resource()->Id;
				case 'Name': return QQN::Resource()->Name;
				default: throw new QCallerException('Simple Property not found in ResourceDataGrid content: ' . $mixContent);
