<?php
	 ...
	 * listing a collection of Giants objects.  It includes
	 ...
	 * 
	 */
	class GiantsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Giants datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Giants object, or it can be any QQNode extending from QQN::Giants().
		 * 
		 ...
		 * sorting using two fields from the Giants object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Giants
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Giants::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Giants, given the clauses above
			$this->DataSource = Giants::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Giants-based QQNode.
		 * It will also verify that it is a proper Giants-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'giants') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "giants".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Giants()->Id;
				case 'Giant': return QQN::Giants()->Giant;
				case 'Country': return QQN::Giants()->Country;
				default: throw new QCallerException('Simple Property not found in GiantsDataGrid content: ' . $mixContent);
