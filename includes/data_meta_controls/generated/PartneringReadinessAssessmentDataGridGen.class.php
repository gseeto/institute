<?php
	 ...
	 * listing a collection of PartneringReadinessAssessment objects.  It includes
	 ...
	 * 
	 */
	class PartneringReadinessAssessmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PartneringReadinessAssessment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PartneringReadinessAssessment object, or it can be any QQNode extending from QQN::PartneringReadinessAssessment().
		 * 
		 ...
		 * sorting using two fields from the PartneringReadinessAssessment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PartneringReadinessAssessment
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PartneringReadinessAssessment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from PartneringReadinessAssessment, given the clauses above
			$this->DataSource = PartneringReadinessAssessment::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PartneringReadinessAssessment-based QQNode.
		 * It will also verify that it is a proper PartneringReadinessAssessment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'partnering_readiness_assessment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "partnering_readiness_assessment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PartneringReadinessAssessment()->Id;
				case 'ResourceStatusId': return QQN::PartneringReadinessAssessment()->ResourceStatusId;
				case 'CompanyId': return QQN::PartneringReadinessAssessment()->CompanyId;
				case 'Company': return QQN::PartneringReadinessAssessment()->Company;
				case 'ResourceId': return QQN::PartneringReadinessAssessment()->ResourceId;
				case 'Resource': return QQN::PartneringReadinessAssessment()->Resource;
				case 'UserId': return QQN::PartneringReadinessAssessment()->UserId;
				case 'User': return QQN::PartneringReadinessAssessment()->User;
				case 'GroupId': return QQN::PartneringReadinessAssessment()->GroupId;
				case 'Group': return QQN::PartneringReadinessAssessment()->Group;
				default: throw new QCallerException('Simple Property not found in PartneringReadinessAssessmentDataGrid content: ' . $mixContent);
