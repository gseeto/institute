<?php
	 ...
	 * listing a collection of LraAssessment objects.  It includes
	 ...
	 * 
	 */
	class LraAssessmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this LraAssessment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the LraAssessment object, or it can be any QQNode extending from QQN::LraAssessment().
		 * 
		 ...
		 * sorting using two fields from the LraAssessment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from LraAssessment
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = LraAssessment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from LraAssessment, given the clauses above
			$this->DataSource = LraAssessment::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a LraAssessment-based QQNode.
		 * It will also verify that it is a proper LraAssessment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'lra_assessment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "lra_assessment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::LraAssessment()->Id;
				case 'ResourceStatusId': return QQN::LraAssessment()->ResourceStatusId;
				case 'ResourceStatus': return QQN::LraAssessment()->ResourceStatus;
				case 'ResourceId': return QQN::LraAssessment()->ResourceId;
				case 'Resource': return QQN::LraAssessment()->Resource;
				case 'UserId': return QQN::LraAssessment()->UserId;
				case 'User': return QQN::LraAssessment()->User;
				case 'GroupId': return QQN::LraAssessment()->GroupId;
				case 'Group': return QQN::LraAssessment()->Group;
				default: throw new QCallerException('Simple Property not found in LraAssessmentDataGrid content: ' . $mixContent);
