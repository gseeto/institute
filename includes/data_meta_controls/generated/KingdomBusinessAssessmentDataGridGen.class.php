<?php
	 ...
	 * listing a collection of KingdomBusinessAssessment objects.  It includes
	 ...
	 * 
	 */
	class KingdomBusinessAssessmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this KingdomBusinessAssessment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the KingdomBusinessAssessment object, or it can be any QQNode extending from QQN::KingdomBusinessAssessment().
		 * 
		 ...
		 * sorting using two fields from the KingdomBusinessAssessment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from KingdomBusinessAssessment
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = KingdomBusinessAssessment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from KingdomBusinessAssessment, given the clauses above
			$this->DataSource = KingdomBusinessAssessment::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a KingdomBusinessAssessment-based QQNode.
		 * It will also verify that it is a proper KingdomBusinessAssessment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'kingdom_business_assessment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "kingdom_business_assessment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::KingdomBusinessAssessment()->Id;
				case 'CompanyId': return QQN::KingdomBusinessAssessment()->CompanyId;
				case 'Company': return QQN::KingdomBusinessAssessment()->Company;
				case 'ResourceId': return QQN::KingdomBusinessAssessment()->ResourceId;
				case 'Resource': return QQN::KingdomBusinessAssessment()->Resource;
				case 'UserId': return QQN::KingdomBusinessAssessment()->UserId;
