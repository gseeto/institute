<?php
	 ...
	 * listing a collection of Company objects.  It includes
	 ...
	 * 
	 */
	class CompanyDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Company datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Company object, or it can be any QQNode extending from QQN::Company().
		 * 
		 ...
		 * sorting using two fields from the Company object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Company
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Company::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Company, given the clauses above
			$this->DataSource = Company::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Company-based QQNode.
		 * It will also verify that it is a proper Company-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'company') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "company".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Company()->Id;
				case 'Name': return QQN::Company()->Name;
				case 'AddressId': return QQN::Company()->AddressId;
				case 'Address': return QQN::Company()->Address;
				default: throw new QCallerException('Simple Property not found in CompanyDataGrid content: ' . $mixContent);
