<?php
	 ...
	 * listing a collection of TimeAssessment objects.  It includes
	 ...
	 * 
	 */
	class TimeAssessmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TimeAssessment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TimeAssessment object, or it can be any QQNode extending from QQN::TimeAssessment().
		 * 
		 ...
		 * sorting using two fields from the TimeAssessment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TimeAssessment
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TimeAssessment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TimeAssessment, given the clauses above
			$this->DataSource = TimeAssessment::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TimeAssessment-based QQNode.
		 * It will also verify that it is a proper TimeAssessment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'time_assessment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "time_assessment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::TimeAssessment()->Id;
				case 'ResourceStatusId': return QQN::TimeAssessment()->ResourceStatusId;
				case 'ResourceStatus': return QQN::TimeAssessment()->ResourceStatus;
				case 'ResourceId': return QQN::TimeAssessment()->ResourceId;
				case 'Resource': return QQN::TimeAssessment()->Resource;
				case 'UserId': return QQN::TimeAssessment()->UserId;
				case 'User': return QQN::TimeAssessment()->User;
				case 'GroupId': return QQN::TimeAssessment()->GroupId;
				case 'Group': return QQN::TimeAssessment()->Group;
				default: throw new QCallerException('Simple Property not found in TimeAssessmentDataGrid content: ' . $mixContent);
