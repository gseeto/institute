<?php
	 ...
	 * listing a collection of TenFQuestions objects.  It includes
	 ...
	 * 
	 */
	class TenFQuestionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TenFQuestions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TenFQuestions object, or it can be any QQNode extending from QQN::TenFQuestions().
		 * 
		 ...
		 * sorting using two fields from the TenFQuestions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TenFQuestions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TenFQuestions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TenFQuestions, given the clauses above
			$this->DataSource = TenFQuestions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TenFQuestions-based QQNode.
		 * It will also verify that it is a proper TenFQuestions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'ten_f_questions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "ten_f_questions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::TenFQuestions()->Id;
				case 'Count': return QQN::TenFQuestions()->Count;
				case 'Text': return QQN::TenFQuestions()->Text;
				case 'CategoryId': return QQN::TenFQuestions()->CategoryId;
				default: throw new QCallerException('Simple Property not found in TenFQuestionsDataGrid content: ' . $mixContent);
