<?php
	 ...
	 * listing a collection of PartneringReadinessResults objects.  It includes
	 ...
	 * 
	 */
	class PartneringReadinessResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PartneringReadinessResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PartneringReadinessResults object, or it can be any QQNode extending from QQN::PartneringReadinessResults().
		 * 
		 ...
		 * sorting using two fields from the PartneringReadinessResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PartneringReadinessResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PartneringReadinessResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from PartneringReadinessResults, given the clauses above
			$this->DataSource = PartneringReadinessResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PartneringReadinessResults-based QQNode.
		 * It will also verify that it is a proper PartneringReadinessResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'partnering_readiness_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "partnering_readiness_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PartneringReadinessResults()->Id;
				case 'QuestionId': return QQN::PartneringReadinessResults()->QuestionId;
				case 'Question': return QQN::PartneringReadinessResults()->Question;
				case 'AssessmentId': return QQN::PartneringReadinessResults()->AssessmentId;
				case 'Assessment': return QQN::PartneringReadinessResults()->Assessment;
				case 'Importance': return QQN::PartneringReadinessResults()->Importance;
				case 'Willingness': return QQN::PartneringReadinessResults()->Willingness;
				default: throw new QCallerException('Simple Property not found in PartneringReadinessResultsDataGrid content: ' . $mixContent);
