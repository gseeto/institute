<?php
	 ...
	 * listing a collection of CannedActionItem objects.  It includes
	 ...
	 * 
	 */
	class CannedActionItemDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CannedActionItem datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CannedActionItem object, or it can be any QQNode extending from QQN::CannedActionItem().
		 * 
		 ...
		 * sorting using two fields from the CannedActionItem object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CannedActionItem
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CannedActionItem::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CannedActionItem, given the clauses above
			$this->DataSource = CannedActionItem::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CannedActionItem-based QQNode.
		 * It will also verify that it is a proper CannedActionItem-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'canned_action_item') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "canned_action_item".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CannedActionItem()->Id;
				case 'Action': return QQN::CannedActionItem()->Action;
				case 'StrategyId': return QQN::CannedActionItem()->StrategyId;
				case 'Strategy': return QQN::CannedActionItem()->Strategy;
				default: throw new QCallerException('Simple Property not found in CannedActionItemDataGrid content: ' . $mixContent);
