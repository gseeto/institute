<?php
	 ...
	 * listing a collection of TenPResults objects.  It includes
	 ...
	 * 
	 */
	class TenPResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TenPResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TenPResults object, or it can be any QQNode extending from QQN::TenPResults().
		 * 
		 ...
		 * sorting using two fields from the TenPResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TenPResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TenPResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TenPResults, given the clauses above
			$this->DataSource = TenPResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TenPResults-based QQNode.
		 * It will also verify that it is a proper TenPResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'ten_p_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "ten_p_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::TenPResults()->Id;
				case 'QuestionId': return QQN::TenPResults()->QuestionId;
				case 'Question': return QQN::TenPResults()->Question;
				case 'AssessmentId': return QQN::TenPResults()->AssessmentId;
				case 'Assessment': return QQN::TenPResults()->Assessment;
				case 'Performance': return QQN::TenPResults()->Performance;
				case 'Importance': return QQN::TenPResults()->Importance;
				default: throw new QCallerException('Simple Property not found in TenPResultsDataGrid content: ' . $mixContent);
