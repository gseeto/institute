<?php
	 ...
	 * listing a collection of UpwardResults objects.  It includes
	 ...
	 * 
	 */
	class UpwardResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this UpwardResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the UpwardResults object, or it can be any QQNode extending from QQN::UpwardResults().
		 * 
		 ...
		 * sorting using two fields from the UpwardResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from UpwardResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = UpwardResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from UpwardResults, given the clauses above
			$this->DataSource = UpwardResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a UpwardResults-based QQNode.
		 * It will also verify that it is a proper UpwardResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'upward_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "upward_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::UpwardResults()->Id;
				case 'QuestionId': return QQN::UpwardResults()->QuestionId;
				case 'Question': return QQN::UpwardResults()->Question;
				case 'AssessmentId': return QQN::UpwardResults()->AssessmentId;
				case 'Assessment': return QQN::UpwardResults()->Assessment;
				case 'Performance': return QQN::UpwardResults()->Performance;
				case 'Importance': return QQN::UpwardResults()->Importance;
				default: throw new QCallerException('Simple Property not found in UpwardResultsDataGrid content: ' . $mixContent);
