<?php
	 ...
	 * listing a collection of TenFResults objects.  It includes
	 ...
	 * 
	 */
	class TenFResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TenFResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TenFResults object, or it can be any QQNode extending from QQN::TenFResults().
		 * 
		 ...
		 * sorting using two fields from the TenFResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TenFResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TenFResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TenFResults, given the clauses above
			$this->DataSource = TenFResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TenFResults-based QQNode.
		 * It will also verify that it is a proper TenFResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'ten_f_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "ten_f_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::TenFResults()->Id;
				case 'QuestionId': return QQN::TenFResults()->QuestionId;
				case 'Question': return QQN::TenFResults()->Question;
				case 'AssessmentId': return QQN::TenFResults()->AssessmentId;
				case 'Assessment': return QQN::TenFResults()->Assessment;
				case 'Performance': return QQN::TenFResults()->Performance;
				case 'Importance': return QQN::TenFResults()->Importance;
				default: throw new QCallerException('Simple Property not found in TenFResultsDataGrid content: ' . $mixContent);
