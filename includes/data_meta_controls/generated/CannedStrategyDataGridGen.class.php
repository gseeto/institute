<?php
	 ...
	 * listing a collection of CannedStrategy objects.  It includes
	 ...
	 * 
	 */
	class CannedStrategyDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CannedStrategy datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CannedStrategy object, or it can be any QQNode extending from QQN::CannedStrategy().
		 * 
		 ...
		 * sorting using two fields from the CannedStrategy object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CannedStrategy
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CannedStrategy::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CannedStrategy, given the clauses above
			$this->DataSource = CannedStrategy::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CannedStrategy-based QQNode.
		 * It will also verify that it is a proper CannedStrategy-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'canned_strategy') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "canned_strategy".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CannedStrategy()->Id;
				case 'Strategy': return QQN::CannedStrategy()->Strategy;
				case 'CategoryTypeId': return QQN::CannedStrategy()->CategoryTypeId;
				default: throw new QCallerException('Simple Property not found in CannedStrategyDataGrid content: ' . $mixContent);
