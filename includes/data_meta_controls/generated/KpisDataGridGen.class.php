<?php
	 ...
	 * listing a collection of Kpis objects.  It includes
	 ...
	 * 
	 */
	class KpisDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Kpis datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Kpis object, or it can be any QQNode extending from QQN::Kpis().
		 * 
		 ...
		 * sorting using two fields from the Kpis object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Kpis
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Kpis::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Kpis, given the clauses above
			$this->DataSource = Kpis::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Kpis-based QQNode.
		 * It will also verify that it is a proper Kpis-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'kpis') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "kpis".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Kpis()->Id;
				case 'StrategyId': return QQN::Kpis()->StrategyId;
				case 'Strategy': return QQN::Kpis()->Strategy;
				case 'ScorecardId': return QQN::Kpis()->ScorecardId;
				case 'Scorecard': return QQN::Kpis()->Scorecard;
				case 'Rating': return QQN::Kpis()->Rating;
				case 'Kpi': return QQN::Kpis()->Kpi;
				case 'Count': return QQN::Kpis()->Count;
				case 'Comments': return QQN::Kpis()->Comments;
				case 'ModifiedBy': return QQN::Kpis()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Kpis()->ModifiedByObject;
				case 'Modified': return QQN::Kpis()->Modified;
				default: throw new QCallerException('Simple Property not found in KpisDataGrid content: ' . $mixContent);
