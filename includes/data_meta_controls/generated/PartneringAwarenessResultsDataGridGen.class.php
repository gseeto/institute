<?php
	 ...
	 * listing a collection of PartneringAwarenessResults objects.  It includes
	 ...
	 * 
	 */
	class PartneringAwarenessResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PartneringAwarenessResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PartneringAwarenessResults object, or it can be any QQNode extending from QQN::PartneringAwarenessResults().
		 * 
		 ...
		 * sorting using two fields from the PartneringAwarenessResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PartneringAwarenessResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PartneringAwarenessResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from PartneringAwarenessResults, given the clauses above
			$this->DataSource = PartneringAwarenessResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PartneringAwarenessResults-based QQNode.
		 * It will also verify that it is a proper PartneringAwarenessResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'partnering_awareness_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "partnering_awareness_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PartneringAwarenessResults()->Id;
				case 'QuestionId': return QQN::PartneringAwarenessResults()->QuestionId;
				case 'Question': return QQN::PartneringAwarenessResults()->Question;
				case 'AssessmentId': return QQN::PartneringAwarenessResults()->AssessmentId;
				case 'Assessment': return QQN::PartneringAwarenessResults()->Assessment;
				case 'Performance': return QQN::PartneringAwarenessResults()->Performance;
				case 'Importance': return QQN::PartneringAwarenessResults()->Importance;
				default: throw new QCallerException('Simple Property not found in PartneringAwarenessResultsDataGrid content: ' . $mixContent);
