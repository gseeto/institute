<?php
	 ...
	 * listing a collection of LemonloversAssessmentResults objects.  It includes
	 ...
	 * 
	 */
	class LemonloversAssessmentResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this LemonloversAssessmentResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the LemonloversAssessmentResults object, or it can be any QQNode extending from QQN::LemonloversAssessmentResults().
		 * 
		 ...
		 * sorting using two fields from the LemonloversAssessmentResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from LemonloversAssessmentResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = LemonloversAssessmentResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from LemonloversAssessmentResults, given the clauses above
			$this->DataSource = LemonloversAssessmentResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a LemonloversAssessmentResults-based QQNode.
		 * It will also verify that it is a proper LemonloversAssessmentResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'lemonlovers_assessment_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "lemonlovers_assessment_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::LemonloversAssessmentResults()->Id;
				case 'QuestionId': return QQN::LemonloversAssessmentResults()->QuestionId;
				case 'Question': return QQN::LemonloversAssessmentResults()->Question;
				case 'AssessmentId': return QQN::LemonloversAssessmentResults()->AssessmentId;
				case 'Value': return QQN::LemonloversAssessmentResults()->Value;
				default: throw new QCallerException('Simple Property not found in LemonloversAssessmentResultsDataGrid content: ' . $mixContent);
