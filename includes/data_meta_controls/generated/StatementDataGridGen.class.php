<?php
	 ...
	 * listing a collection of Statement objects.  It includes
	 ...
	 * 
	 */
	class StatementDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Statement datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Statement object, or it can be any QQNode extending from QQN::Statement().
		 * 
		 ...
		 * sorting using two fields from the Statement object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Statement
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Statement::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Statement, given the clauses above
			$this->DataSource = Statement::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Statement-based QQNode.
		 * It will also verify that it is a proper Statement-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'statement') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "statement".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Statement()->Id;
				case 'Value': return QQN::Statement()->Value;
				case 'ScorecardId': return QQN::Statement()->ScorecardId;
				case 'Scorecard': return QQN::Statement()->Scorecard;
				case 'IsPurpose': return QQN::Statement()->IsPurpose;
				case 'ModifiedBy': return QQN::Statement()->ModifiedBy;
				case 'ModifiedByObject': return QQN::Statement()->ModifiedByObject;
				case 'Modified': return QQN::Statement()->Modified;
				default: throw new QCallerException('Simple Property not found in StatementDataGrid content: ' . $mixContent);
