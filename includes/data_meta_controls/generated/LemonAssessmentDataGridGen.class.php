<?php
	 ...
	 * listing a collection of LemonAssessment objects.  It includes
	 ...
	 * 
	 */
	class LemonAssessmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this LemonAssessment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the LemonAssessment object, or it can be any QQNode extending from QQN::LemonAssessment().
		 * 
		 ...
		 * sorting using two fields from the LemonAssessment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from LemonAssessment
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = LemonAssessment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from LemonAssessment, given the clauses above
			$this->DataSource = LemonAssessment::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a LemonAssessment-based QQNode.
		 * It will also verify that it is a proper LemonAssessment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'lemon_assessment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "lemon_assessment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::LemonAssessment()->Id;
				case 'UserId': return QQN::LemonAssessment()->UserId;
				case 'User': return QQN::LemonAssessment()->User;
				case 'CompanyId': return QQN::LemonAssessment()->CompanyId;
				case 'Company': return QQN::LemonAssessment()->Company;
				case 'ResourceId': return QQN::LemonAssessment()->ResourceId;
				case 'Resource': return QQN::LemonAssessment()->Resource;
