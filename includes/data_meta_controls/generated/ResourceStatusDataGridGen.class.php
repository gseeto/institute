<?php
	 ...
	 * listing a collection of ResourceStatus objects.  It includes
	 ...
	 * 
	 */
	class ResourceStatusDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ResourceStatus datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ResourceStatus object, or it can be any QQNode extending from QQN::ResourceStatus().
		 * 
		 ...
		 * sorting using two fields from the ResourceStatus object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ResourceStatus
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ResourceStatus::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from ResourceStatus, given the clauses above
			$this->DataSource = ResourceStatus::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ResourceStatus-based QQNode.
		 * It will also verify that it is a proper ResourceStatus-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'resource_status') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "resource_status".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ResourceStatus()->Id;
				case 'Value': return QQN::ResourceStatus()->Value;
				default: throw new QCallerException('Simple Property not found in ResourceStatusDataGrid content: ' . $mixContent);
