<?php
	 ...
	 * listing a collection of LemonAssessmentQuestions objects.  It includes
	 ...
	 * 
	 */
	class LemonAssessmentQuestionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this LemonAssessmentQuestions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the LemonAssessmentQuestions object, or it can be any QQNode extending from QQN::LemonAssessmentQuestions().
		 * 
		 ...
		 * sorting using two fields from the LemonAssessmentQuestions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from LemonAssessmentQuestions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = LemonAssessmentQuestions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from LemonAssessmentQuestions, given the clauses above
			$this->DataSource = LemonAssessmentQuestions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a LemonAssessmentQuestions-based QQNode.
		 * It will also verify that it is a proper LemonAssessmentQuestions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'lemon_assessment_questions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "lemon_assessment_questions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::LemonAssessmentQuestions()->Id;
				case 'Count': return QQN::LemonAssessmentQuestions()->Count;
				case 'Text': return QQN::LemonAssessmentQuestions()->Text;
				case 'LemonTypeId': return QQN::LemonAssessmentQuestions()->LemonTypeId;
				case 'Inverse': return QQN::LemonAssessmentQuestions()->Inverse;
				default: throw new QCallerException('Simple Property not found in LemonAssessmentQuestionsDataGrid content: ' . $mixContent);
