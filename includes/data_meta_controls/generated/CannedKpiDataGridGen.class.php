<?php
	 ...
	 * listing a collection of CannedKpi objects.  It includes
	 ...
	 * 
	 */
	class CannedKpiDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CannedKpi datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CannedKpi object, or it can be any QQNode extending from QQN::CannedKpi().
		 * 
		 ...
		 * sorting using two fields from the CannedKpi object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CannedKpi
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CannedKpi::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CannedKpi, given the clauses above
			$this->DataSource = CannedKpi::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CannedKpi-based QQNode.
		 * It will also verify that it is a proper CannedKpi-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'canned_kpi') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "canned_kpi".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CannedKpi()->Id;
				case 'StrategyId': return QQN::CannedKpi()->StrategyId;
				case 'Strategy': return QQN::CannedKpi()->Strategy;
				case 'Kpi': return QQN::CannedKpi()->Kpi;
				default: throw new QCallerException('Simple Property not found in CannedKpiDataGrid content: ' . $mixContent);
