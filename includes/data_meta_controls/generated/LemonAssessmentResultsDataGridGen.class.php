<?php
	 ...
	 * listing a collection of LemonAssessmentResults objects.  It includes
	 ...
	 * 
	 */
	class LemonAssessmentResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this LemonAssessmentResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the LemonAssessmentResults object, or it can be any QQNode extending from QQN::LemonAssessmentResults().
		 * 
		 ...
		 * sorting using two fields from the LemonAssessmentResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from LemonAssessmentResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = LemonAssessmentResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from LemonAssessmentResults, given the clauses above
			$this->DataSource = LemonAssessmentResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a LemonAssessmentResults-based QQNode.
		 * It will also verify that it is a proper LemonAssessmentResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'lemon_assessment_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "lemon_assessment_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::LemonAssessmentResults()->Id;
				case 'QuestionId': return QQN::LemonAssessmentResults()->QuestionId;
				case 'Question': return QQN::LemonAssessmentResults()->Question;
				case 'AssessmentId': return QQN::LemonAssessmentResults()->AssessmentId;
				case 'Assessment': return QQN::LemonAssessmentResults()->Assessment;
				case 'Value': return QQN::LemonAssessmentResults()->Value;
				default: throw new QCallerException('Simple Property not found in LemonAssessmentResultsDataGrid content: ' . $mixContent);
