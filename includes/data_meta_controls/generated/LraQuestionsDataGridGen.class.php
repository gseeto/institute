<?php
	 ...
	 * listing a collection of LraQuestions objects.  It includes
	 ...
	 * 
	 */
	class LraQuestionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this LraQuestions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the LraQuestions object, or it can be any QQNode extending from QQN::LraQuestions().
		 * 
		 ...
		 * sorting using two fields from the LraQuestions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from LraQuestions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = LraQuestions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from LraQuestions, given the clauses above
			$this->DataSource = LraQuestions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a LraQuestions-based QQNode.
		 * It will also verify that it is a proper LraQuestions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'lra_questions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "lra_questions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::LraQuestions()->Id;
				case 'Count': return QQN::LraQuestions()->Count;
				case 'Text': return QQN::LraQuestions()->Text;
				case 'CategoryId': return QQN::LraQuestions()->CategoryId;
				case 'Invert': return QQN::LraQuestions()->Invert;
				default: throw new QCallerException('Simple Property not found in LraQuestionsDataGrid content: ' . $mixContent);
