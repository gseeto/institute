<?php
	 ...
	 * listing a collection of PartneringReadinessQuestions objects.  It includes
	 ...
	 * 
	 */
	class PartneringReadinessQuestionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PartneringReadinessQuestions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PartneringReadinessQuestions object, or it can be any QQNode extending from QQN::PartneringReadinessQuestions().
		 * 
		 ...
		 * sorting using two fields from the PartneringReadinessQuestions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PartneringReadinessQuestions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PartneringReadinessQuestions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from PartneringReadinessQuestions, given the clauses above
			$this->DataSource = PartneringReadinessQuestions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PartneringReadinessQuestions-based QQNode.
		 * It will also verify that it is a proper PartneringReadinessQuestions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'partnering_readiness_questions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "partnering_readiness_questions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PartneringReadinessQuestions()->Id;
				case 'Count': return QQN::PartneringReadinessQuestions()->Count;
				case 'Text': return QQN::PartneringReadinessQuestions()->Text;
				case 'CategoryId': return QQN::PartneringReadinessQuestions()->CategoryId;
				default: throw new QCallerException('Simple Property not found in PartneringReadinessQuestionsDataGrid content: ' . $mixContent);
