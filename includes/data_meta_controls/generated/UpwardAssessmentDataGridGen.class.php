<?php
	 ...
	 * listing a collection of UpwardAssessment objects.  It includes
	 ...
	 * 
	 */
	class UpwardAssessmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this UpwardAssessment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the UpwardAssessment object, or it can be any QQNode extending from QQN::UpwardAssessment().
		 * 
		 ...
		 * sorting using two fields from the UpwardAssessment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from UpwardAssessment
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = UpwardAssessment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from UpwardAssessment, given the clauses above
			$this->DataSource = UpwardAssessment::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a UpwardAssessment-based QQNode.
		 * It will also verify that it is a proper UpwardAssessment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'upward_assessment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "upward_assessment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::UpwardAssessment()->Id;
				case 'ResourceStatusId': return QQN::UpwardAssessment()->ResourceStatusId;
				case 'CompanyId': return QQN::UpwardAssessment()->CompanyId;
				case 'ResourceId': return QQN::UpwardAssessment()->ResourceId;
				case 'UserId': return QQN::UpwardAssessment()->UserId;
				case 'User': return QQN::UpwardAssessment()->User;
				case 'GroupId': return QQN::UpwardAssessment()->GroupId;
				case 'Group': return QQN::UpwardAssessment()->Group;
				default: throw new QCallerException('Simple Property not found in UpwardAssessmentDataGrid content: ' . $mixContent);
