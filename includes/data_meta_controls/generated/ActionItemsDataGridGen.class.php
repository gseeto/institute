<?php
	 ...
	 * listing a collection of ActionItems objects.  It includes
	 ...
	 * 
	 */
	class ActionItemsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ActionItems datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ActionItems object, or it can be any QQNode extending from QQN::ActionItems().
		 * 
		 ...
		 * sorting using two fields from the ActionItems object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ActionItems
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ActionItems::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from ActionItems, given the clauses above
			$this->DataSource = ActionItems::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ActionItems-based QQNode.
		 * It will also verify that it is a proper ActionItems-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'action_items') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "action_items".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ActionItems()->Id;
				case 'Action': return QQN::ActionItems()->Action;
				case 'StrategyId': return QQN::ActionItems()->StrategyId;
				case 'Strategy': return QQN::ActionItems()->Strategy;
				case 'ScorecardId': return QQN::ActionItems()->ScorecardId;
				case 'Scorecard': return QQN::ActionItems()->Scorecard;
				case 'Who': return QQN::ActionItems()->Who;
				case 'WhoObject': return QQN::ActionItems()->WhoObject;
				case 'When': return QQN::ActionItems()->When;
				case 'StatusType': return QQN::ActionItems()->StatusType;
				case 'Comments': return QQN::ActionItems()->Comments;
				case 'CategoryId': return QQN::ActionItems()->CategoryId;
				case 'Count': return QQN::ActionItems()->Count;
