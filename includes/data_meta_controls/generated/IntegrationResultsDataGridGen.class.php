<?php
	 ...
	 * listing a collection of IntegrationResults objects.  It includes
	 ...
	 * 
	 */
	class IntegrationResultsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this IntegrationResults datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the IntegrationResults object, or it can be any QQNode extending from QQN::IntegrationResults().
		 * 
		 ...
		 * sorting using two fields from the IntegrationResults object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from IntegrationResults
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = IntegrationResults::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from IntegrationResults, given the clauses above
			$this->DataSource = IntegrationResults::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a IntegrationResults-based QQNode.
		 * It will also verify that it is a proper IntegrationResults-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'integration_results') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "integration_results".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::IntegrationResults()->Id;
				case 'QuestionId': return QQN::IntegrationResults()->QuestionId;
				case 'Question': return QQN::IntegrationResults()->Question;
				case 'AssessmentId': return QQN::IntegrationResults()->AssessmentId;
				case 'Assessment': return QQN::IntegrationResults()->Assessment;
				case 'Result': return QQN::IntegrationResults()->Result;
				default: throw new QCallerException('Simple Property not found in IntegrationResultsDataGrid content: ' . $mixContent);
