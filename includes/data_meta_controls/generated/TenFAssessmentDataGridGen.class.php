<?php
	 ...
	 * listing a collection of TenFAssessment objects.  It includes
	 ...
	 * 
	 */
	class TenFAssessmentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this TenFAssessment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TenFAssessment object, or it can be any QQNode extending from QQN::TenFAssessment().
		 * 
		 ...
		 * sorting using two fields from the TenFAssessment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TenFAssessment
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = TenFAssessment::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from TenFAssessment, given the clauses above
			$this->DataSource = TenFAssessment::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TenFAssessment-based QQNode.
		 * It will also verify that it is a proper TenFAssessment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'ten_f_assessment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "ten_f_assessment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::TenFAssessment()->Id;
				case 'ResourceStatusId': return QQN::TenFAssessment()->ResourceStatusId;
				case 'ResourceStatus': return QQN::TenFAssessment()->ResourceStatus;
				case 'ResourceId': return QQN::TenFAssessment()->ResourceId;
				case 'Resource': return QQN::TenFAssessment()->Resource;
				case 'UserId': return QQN::TenFAssessment()->UserId;
				case 'User': return QQN::TenFAssessment()->User;
				case 'GroupId': return QQN::TenFAssessment()->GroupId;
				case 'Group': return QQN::TenFAssessment()->Group;
				default: throw new QCallerException('Simple Property not found in TenFAssessmentDataGrid content: ' . $mixContent);
