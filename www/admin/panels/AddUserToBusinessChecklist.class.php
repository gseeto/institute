<?php
    class AddUserToBusinessChecklist extends QPanel {
      ...
      //  protected $strBackColor = '#fefece';
        ...
        public function __construct($objParentObject, $strMethodCallBack, $objParent, $strControlId = null) {
			...
			foreach($checklistArray as $objChecklist) {
				$this->lstChecklist->AddItem($objChecklist->Company->Name, $objChecklist->Id);
			}
    ...
    public function btnCancel_Click($strFormId, $strControlId, $strParameter) {
     ...
     public function btnSubmit_Click($strFormId, $strControlId, $strParameter) {
		...
		$objCheckList = BusinessChecklist::Load($this->lstChecklist->SelectedValue);
	     foreach($this->selectedUserArray as $intUserId) {				
		     	$objUser = User::Load($intUserId);
		     	if(!$objCheckList->IsUserAsManagerAssociated($objUser))
		     		$objCheckList->AssociateUserAsManager($objUser);
		     	$objCheckList->Save();
			}
