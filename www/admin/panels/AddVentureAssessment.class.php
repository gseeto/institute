<?php
    class AddVentureAssessment extends QPanel {
        ...
        protected $strTemplate = '../../admin/panels/AddVentureAssessment.tpl.php';
      ...
      //  protected $strBackColor = '#fefece';
        ...
        public function __construct($objParentObject, $strMethodCallBack, $objParent, $strControlId = null) {
			...
			$this->btnSubmit->Text = "Add Users to Post Venture Assessment";
     ...
     public function btnSubmit_Click($strFormId, $strControlId, $strParameter) {
		...
		foreach($this->selectedUserArray as $intUserId) {
			// Create a new assessment entry first
	     	$objAssessment = new PostventureAssessment();
	     	$objAssessment->UserId = $intUserId;
	     	$objAssessment->ResourceId = 11; //Post Venture Assessment - going to have to find a nicer way of doing this
	     	...
	     	$objUser->AssociateResource(Resource::Load(11));
			$objAssessment->Save();
		}
		$this->Visible = false;
	...
	public function dtgUsers_Bind() {
		...
		foreach($userArray as $objUser) {
			if(!$objUser->IsResourceAssociated(Resource::Load(11))) {
