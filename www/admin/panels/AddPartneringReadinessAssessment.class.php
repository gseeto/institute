<?php
    class AddPartneringReadinessAssessment extends QPanel {
        ...
        protected $strTemplate = '../../admin/panels/AddPartneringReadinessAssessment.tpl.php';
      ...
      //  protected $strBackColor = '#fefece';
        ...
        public function __construct($objParentObject, $strMethodCallBack, $objParent, $strControlId = null) {
            ...
            $this->dtgUsers->AddColumn(new QDataGridColumn('Group', '<?= $_CONTROL->ParentControl->txtGroup_Render($_ITEM) ?>', 'HtmlEntities=false' ));
			...
			$this->btnSubmit->Text = "Add Users to Partnering Readiness Assessment";
     ...
     public function btnSubmit_Click($strFormId, $strControlId, $strParameter) {
		...
		foreach($this->selectedUserArray as $intUserId) {
			// Create a new assessment entry first
	     	$objAssessment = new PartneringReadinessAssessment();
	     	$objAssessment->UserId = $intUserId;
	     	$objAssessment->ResourceId = 13; //Partner Readiness Assessment - going to have to find a nicer way of doing this
	     	$objAssessment->ResourceStatusId = 1; // initial state is untouched
	     	
	     	$strControlId = 'txtGroup' . $intUserId;
	     	$txtGroup = $this->objForm->GetControl($strControlId);
	     	if($txtGroup != null) {
	     		$objGroup = GroupAssessmentList::LoadByKeyCode($txtGroup->Text);
	     		if($objGroup != null)
	     			$objAssessment->GroupId = $objGroup->Id;
	     		else {
	     			$objGroup = new GroupAssessmentList();
	     			$objGroup->KeyCode = $txtGroup->Text;
	     			$objGroup->KeysLeft = 1;
	     			$objGroup->TotalKeys = 1;
	     			$objGroup->ResourceId = 13;
	     			$groupId = $objGroup->Save();
	     			$objAssessment->GroupId = $groupId;
	     		}
	     	}
	     	$objUser = User::Load($intUserId);
	     	
	     	$objUser->AssociateResource(Resource::Load(13));
			$objAssessment->Save();
		}
		$this->Visible = false;
	...
	public function txtGroup_Render(User $objUser) {
        ...
        if (!$txtGroup) {
            ...
            $txtGroup->Name = "Group:";
			$txtGroup->Width = 100;
			$txtGroup->CssClass = 'form-control';
        }
    ...
    public function chkSelected_Render(User $objUser) {
     	$strControlId = 'chkSelectedReady' . $objUser->Id;
	...
	public function dtgUsers_Bind() {
		...
		$filteredUserArray = array();
		$userArray = User::QueryArray($objConditions,$objClauses);
		foreach($userArray as $objUser) {
			if(!$objUser->IsResourceAssociated(Resource::Load(13))) {
				$filteredUserArray[] = $objUser;
			}
		}	
		$this->dtgUsers->DataSource = $filteredUserArray; 
	}
