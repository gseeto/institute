<?php
    class AddPartneringAwarenessAssessment extends QPanel {
        ...
        protected $strTemplate = '../../admin/panels/AddPartneringAwarenessAssessment.tpl.php';
        ...
        public function __construct($objParentObject, $strMethodCallBack, $objParent, $strControlId = null) {
			...
			$this->btnSubmit->Text = "Add Users to Partnering Awareness Assessment";
     ...
     public function btnSubmit_Click($strFormId, $strControlId, $strParameter) {
		...
		foreach($this->selectedUserArray as $intUserId) {
			// Create a new assessment entry first
	     	$objAssessment = new PartneringAwarenessAssessment();
	     	$objAssessment->UserId = $intUserId;
	     	$objAssessment->ResourceId = 12; //Partner Awareness Assessment - going to have to find a nicer way of doing this
	     	...
	     	$objUser->AssociateResource(Resource::Load(12));
			$objAssessment->Save();
		}
		$this->Visible = false;
	...
	public function dtgUsers_Bind() {
		...
		foreach($userArray as $objUser) {
			if(!$objUser->IsResourceAssociated(Resource::Load(12))) {
