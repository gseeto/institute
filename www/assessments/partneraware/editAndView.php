...
class EditViewPAwarenessAssessmentForm extends InstituteForm {
	...
	protected function Form_Create() {	
		...
		$this->arrayImportance = array();
	 	...
	 	for($i=0; $i<10;$i++) {
	 		$this->dtgAssessmentQuestionArray[$i] = new PartneringAwarenessQuestionsDataGrid($this);
	...
	protected function btnNext_Click() {
		switch($this->iCounter) {
			...
			case 9:
				$this->saveResults();
				QApplication::Redirect(__SUBDIRECTORY__.'/assessments/partneraware/viewAssessment.php');
				break;
		}
	}
	...
	protected function btnSubmit_Click() {	
        $this->saveResults();
        if(!$this->bEditing) {
	        $this->objPAwarenessAssessment->ResourceStatusId = 2;
	        $this->objPAwarenessAssessment->Save();
        }
		QApplication::Redirect(__SUBDIRECTORY__.'/assessments/partneraware/viewAssessment.php');
	}
	...
	protected function saveResults() {
		for($i=0;$i<count($this->arrayPerformance); $i++) {
			$objResults = PartneringAwarenessResults::LoadResultByAssessmentIdAndQuestionId($this->objPAwarenessAssessment->Id, $i+1);
			if($objResults == null){
				$objResults = new PartneringAwarenessResults();
			}
		/*	if($this->bEditing) {
	        	$objResults = PartneringAwarenessResults::LoadResultByAssessmentIdAndQuestionId($this->objPAwarenessAssessment->Id, $i+1);
	        } else {
        ...
        */		
	        $objResults->AssessmentId = $this->objPAwarenessAssessment->Id;  
...
public function lstImportance_Render(PartneringAwarenessQuestions $objQuestions) {
        ...
        if (!$lstImportance) {
            ...
            if($this->bEditing) {
            	$value = PartneringAwarenessResults::GetImportanceByAssessmentIdAndQuestionId($this->objPAwarenessAssessment->Id, $objQuestions->Id);
	            for ($i=1; $i<8; $i++) {
	            	if ($value == $i) 
...
public function lstPerformance_Render(PartneringAwarenessQuestions $objQuestions) {
        // In order to keep track we will use explicitly defined control ids.
        ...
        if (!$lstPerformance) {
        	...
        	// Initialize values from previous assessment
            if($this->bEditing) {
            	$value = PartneringAwarenessResults::GetPerformanceByAssessmentIdAndQuestionId($this->objPAwarenessAssessment->Id, $objQuestions->Id);
	            for ($i=1; $i<8; $i++) {
	            	if ($value == $i) 
	...
	protected function updateChart() {
		...
		foreach(CategoryType::$NameArray as $key=>$value) {			
			$resultArray = PartneringAwarenessResults::LoadArrayByAssessmentIdAndCategory($this->objPAwarenessAssessment->Id,$key);
			$ptotal = $itotal = 0;
...
EditViewPAwarenessAssessmentForm::Run('EditViewPAwarenessAssessmentForm');
