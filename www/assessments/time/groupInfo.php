...
class TimeGroupInfoForm extends InstituteForm {
	...
	protected function btnSubmit_Click() {
		...
		if ($objLogin) {
			...
			// Create a new assessment entry associated with the user
	     	...
	     	$objAssessment->ResourceId = 8; //Integration Assessment - going to have to find a nicer way of doing this
			...
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/time/newAssessment');
		}
	}
	...
	protected function btnLogin_Click() {
		...
		if ($objUser->IsResourceAssociated(Resource::Load(8))) {
			QApplication::Login($objLogin);
			$objAssessmentArray = TimeAssessment::LoadArrayByUserId($objLogin->Id);
			$objAssessment = $objAssessmentArray[0];
			$objAssessment->GroupId = $this->intGroupAssessment; 
			$objAssessment->Save();
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/time/newAssessment/edit');
		} else {
	     	$objAssessment = new TimeAssessment();
	     	$objAssessment->UserId = $objUser->Id;
	     	$objAssessment->ResourceId = 8; //TimeAssessment
	     	$objAssessment->ResourceStatusId = 1; // initial state is untouched
	     	$objAssessment->GroupId = $this->intGroupAssessment;  	
	     	$objUser->AssociateResource(Resource::Load(8));
			$objAssessment->Save();
			QApplication::Login($objLogin);
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/time/newAssessment.php');
		}
	}
	
}

TimeGroupInfoForm::Run('TimeGroupInfoForm');
?>