...
class LRAGroupInfoForm extends InstituteForm {
	...
	protected function btnSubmit_Click() {
		...
		if ($objLogin) {
			...
			// Create a new assessment entry associated with the user
	     	...
	     	$objAssessment->ResourceId = 9; //LRA Assessment - going to have to find a nicer way of doing this
			...
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/lra/groupQuestions.php');
		}
	...
	protected function btnLogin_Click() {
		...
		if ($objUser->IsResourceAssociated(Resource::Load(9))) {
			$assessmentArray = LraAssessment::LoadArrayByUserId($objUser->Id);
			$objLRAAssessment = $assessmentArray[0];
			$objLRAAssessment->GroupId = $this->intGroupAssessment; // make sure the group id is set
			$objLRAAssessment->Save();
			QApplication::Login($objLogin);
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/lra/groupQuestions.php/edit');
		} else {
	     	$objAssessment = new LraAssessment();
	     	$objAssessment->UserId = $objUser->Id;
	     	$objAssessment->ResourceId = 9; 
	     	$objAssessment->ResourceStatusId = 1; // initial state is untouched
	     	$objAssessment->GroupId = $this->intGroupAssessment;  	
	     	$objUser->AssociateResource(Resource::Load(9));
			$objAssessment->Save();
...
LRAGroupInfoForm::Run('LRAGroupInfoForm');
?>