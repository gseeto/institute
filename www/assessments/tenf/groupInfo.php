...
class FGroupInfoForm extends InstituteForm {
	...
	protected function btnSubmit_Click() {
		...
		if ($objLogin) {
			...
			// Create a new assessment entry associated with the user
	     	...
	     	$objAssessment->ResourceId = 3; //10-F Assessment - going to have to find a nicer way of doing this
			...
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/tenf/groupQuestions.php');
		}
	...
	protected function btnLogin_Click() {
		...
		if ($objUser->IsResourceAssociated(Resource::Load(3))) {
			$assessmentArray = TenFAssessment::LoadArrayByUserId($objUser->Id);
			$objTenFAssessment = $assessmentArray[0];
			$objTenFAssessment->GroupId = $this->intGroupAssessment; // make sure the group id is set
			$objTenFAssessment->Save();
			QApplication::Login($objLogin);
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/tenf/groupQuestions.php/edit');
		} else {
	     	$objAssessment = new TenFAssessment();
	     	$objAssessment->UserId = $objUser->Id;
...
FGroupInfoForm::Run('FGroupInfoForm');
?>