...
class ViewPostVentureAssessmentForm extends InstituteForm {
	...
	protected function Form_Create() {
		...
		if($intUserId) { //show the assessment specified
			...
			$this->lblIntroduction->Text = 'Post Venture Assessment for '.$objUser->FirstName. ' '.$objUser->LastName;
		} else { // show the user's assessment	
			$this->lblIntroduction->Text = 'Thank you for taking the Post Venture Assessment.
Your results are provided below.';
		...
		for($i=0; $i<10;$i++) {
	 		$this->dtgAssessmentResultsArray[$i] = new PostventureResultsDataGrid($this);
			...
			$assessmentArray = PostventureResults::LoadArrayByAssessmentIdAndCategory($this->objPostVentureAssessment->Id, $i+1);					
    ...
    public function RenderQuestion($intQuestionId) {
    	$objQuestion = PostventureQuestions::Load($intQuestionId);
	...
	protected function initializeChart() {
		...
		foreach(CategoryType::$NameArray as $key=>$value) {			
			$resultArray = PostventureResults::LoadArrayByAssessmentIdAndCategory($this->objPostVentureAssessment->Id,$key);
			$ptotal = $itotal = 0;
	...
	protected function btnGeneratePdf_Click() { 
		...
		$objPage1->drawText("Post Venture Assessment Report", 150, $PageHeight, 'UTF-8');
		...
		$yPos -= $lineHeight;	
		$ZendImage = Zend_Pdf_Image::imageWithPath( __UPLOAD_DIR__.'/postventure' . $this->objPostVentureAssessment->Id. '.png');		
		...
		for($i=0; $i<5;$i++) {
			...
			$yPos -= ($lineHeight+5);
			$objPage3->setFillColor(Zend_Pdf_Color_Html::color('#79b9c9'))->drawRectangle($col1, $yPos, $col5, $yPos-20, Zend_Pdf_Page::SHAPE_DRAW_FILL_AND_STROKE);
			$objPage3->drawLine($col1, $yPos, $col1, $yPos-$tableheight);
			$objPage3->drawLine($col2, $yPos, $col2, $yPos-$tableheight);
			$objPage3->drawLine($col3, $yPos, $col3, $yPos-$tableheight);
			$objPage3->drawLine($col4, $yPos, $col4, $yPos-$tableheight);
			$objPage3->drawLine($col5, $yPos, $col5, $yPos-$tableheight);
			$objPage3->drawLine($col1, $yPos-$tableheight, $col5, $yPos-$tableheight);
			$objPage3->setFillColor(Zend_Pdf_Color_Html::color('#000000'));
			$yPos -= $lineHeight;
			$objPage3->setFont(Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA_BOLD), 12);
			$objPage3->drawText("Question", $col2+10, $yPos, 'UTF-8');
			$objPage3->drawText("Importance", $col3+2, $yPos, 'UTF-8');
			$objPage3->drawText("Performance", $col4+2, $yPos, 'UTF-8');
			$yPos -= ($lineHeight+5);
			...
			foreach($assessmentArray as $objQuestion){
				...
				$strStatement = PostventureQuestions::Load($objQuestion->QuestionId)->Text;
				if(strlen($strStatement)>100) {
					$objPage3->drawText(substr($strStatement,0,100), $col2+2, $yPos, 'UTF-8');
					$objPage3->drawText(substr($strStatement,100), $col2+2, $yPos-($lineHeight+2), 'UTF-8');
				} else {
		...
		for($i=5; $i<10;$i++) {
			...
			$yPos -= ($lineHeight+5);
			$objPage4->setFillColor(Zend_Pdf_Color_Html::color('#79b9c9'))->drawRectangle($col1, $yPos, $col5, $yPos-20, Zend_Pdf_Page::SHAPE_DRAW_FILL_AND_STROKE);
			$objPage4->drawLine($col1, $yPos, $col1, $yPos-$tableheight);
			$objPage4->drawLine($col2, $yPos, $col2, $yPos-$tableheight);
			$objPage4->drawLine($col3, $yPos, $col3, $yPos-$tableheight);
			$objPage4->drawLine($col4, $yPos, $col4, $yPos-$tableheight);
			$objPage4->drawLine($col5, $yPos, $col5, $yPos-$tableheight);
			$objPage4->drawLine($col1, $yPos-$tableheight, $col5, $yPos-$tableheight);
			$objPage4->setFillColor(Zend_Pdf_Color_Html::color('#000000'));
			$yPos -= $lineHeight;
			$objPage4->setFont(Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA_BOLD), 12);
			$objPage4->drawText("Question", $col2+10, $yPos, 'UTF-8');
			$objPage4->drawText("Importance", $col3+2, $yPos, 'UTF-8');
			$objPage4->drawText("Performance", $col4+2, $yPos, 'UTF-8');
			$yPos -= ($lineHeight+5);
		...
		$pdfFile = '/postventure' . $this->objPostVentureAssessment->Id .rand(0,50). '.pdf';
...
ViewPostVentureAssessmentForm::Run('ViewPostVentureAssessmentForm');
class tenPArray {
			public $P;
			public $importance;
			public $performance;
