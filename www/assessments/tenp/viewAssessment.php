...
Your results are provided below.';
	...
	protected function btnGeneratePdf_Click() { 
		...
		$objPage1->drawText("10 P Assessment Report", 150, $PageHeight, 'UTF-8');
		...
		$yPos -= $lineHeight;	
		$ZendImage = Zend_Pdf_Image::imageWithPath( __UPLOAD_DIR__.'/tenp' . $this->objTenPAssessment->Id. '.png');		
		...
		for($i=0; $i<5;$i++) {
			...
			foreach($assessmentArray as $objQuestion){
				...
				$strStatement = TenPQuestions::Load($objQuestion->QuestionId)->Text;
		...
		$pdfFile = '/tenp' . $this->objTenPAssessment->Id .rand(0,50). '.pdf';
