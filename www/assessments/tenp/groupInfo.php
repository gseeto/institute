...
class PGroupInfoForm extends InstituteForm {
	...
	protected function btnSubmit_Click() {
		...
		if ($objLogin) {
			...
			// Create a new assessment entry associated with the user
	     	...
	     	$objAssessment->ResourceId = 2; //10-P Assessment - going to have to find a nicer way of doing this
	...
	protected function btnLogin_Click() {
		...
		if ($objUser->IsResourceAssociated(Resource::Load(2))) {
			...
			if(!empty($objResultArray))
	     	...
	     	$objAssessment = new TenPAssessment();
	     	$objAssessment->UserId = $objUser->Id;
	     	$objAssessment->ResourceId = 2; //LemonAssessment - going to have to find a nicer way of doing this
	     	$objAssessment->ResourceStatusId = 1; // initial state is untouched
	     	$objAssessment->GroupId = $this->intGroupAssessment;  	
	     	$objUser->AssociateResource(Resource::Load(2));
			$objAssessment->Save();
...
PGroupInfoForm::Run('PGroupInfoForm');
?>