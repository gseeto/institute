...
class KIAGroupInfoForm extends InstituteForm {
	...
	protected function btnSubmit_Click() {
		...
		if ($objLogin) {
			...
			// Create a new assessment entry associated with the user
	     	$objAssessment = new KingdomBusinessAssessment();
	     	$objAssessment->UserId = $objUser->Id;
	     	$objAssessment->ResourceId = 4; //Kingdom Business Assessment - going to have to find a nicer way of doing this
			...
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/kingdom/groupQuestions.php');
		}
	}
	...
	protected function btnLogin_Click() {
		...
		if ($objUser->IsResourceAssociated(Resource::Load(4))) {
			$assessmentArray = KingdomBusinessAssessment::LoadArrayByUserId($objUser->Id);
			$objKingdomAssessment = $assessmentArray[0];
			$objKingdomAssessment->GroupId = $this->intGroupAssessment; // if it's an existing assessment make sure group Id is reassigned correctly
			$objKingdomAssessment->Save();
			QApplication::Login($objLogin);
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/kingdom/groupQuestions.php/edit');
		} else {
	     	$objAssessment = new KingdomBusinessAssessment();
	     	$objAssessment->UserId = $objUser->Id;
	     	$objAssessment->ResourceId = 4; //Kingdom Business Assessment - going to have to find a nicer way of doing this
...
KIAGroupInfoForm::Run('KIAGroupInfoForm');
?>