...
class View50LemonAssessmentForm extends InstituteForm {
	...
	$this->lblBadDay->HtmlEntities = false;
		...
		if($intUserId) { //show the assessment specified
			$assessmentArray = Lemon50Assessment::LoadArrayByUserId($intUserId);
		...
		/*		
		$this->dtgAssessmentResults = new Lemon50AssessmentResultsDataGrid($this);
		...
		$this->dtgAssessmentResults->NoDataHtml = 'Your individual Assessment answers have not been recorded. You would have to retake the test in order for us to display them.';
	...
	protected function getLemonDescription() {
		$lemonValues = array();
		$key = '';
		$resultArray = Lemon50AssessmentResults::LoadArrayByAssessmentId($this->objLemonAssessment->Id);
		if(null != $resultArray) {
			...
			// cater for generated LEMONs who might not have Results Values
			$lemonValues['Luminary'] = $this->objLemonAssessment->L;
			$lemonValues['Entrepreneur'] = $this->objLemonAssessment->E;
			$lemonValues['Manager'] = $this->objLemonAssessment->M;
			$lemonValues['Organizer'] = $this->objLemonAssessment->O;
			$lemonValues['Networker'] = $this->objLemonAssessment->N;			
		}
				...
				switch($key) {
					case 'Luminary':
						...
						'<h4>Characteristics</h4>
						 ...
						 statement about what this means for future.
						 </p>
						 <h4>Work</h4>
					...
					case 'Networker':
						...
						'<h4>Characteristics</h4>
						   ...
						   objectives and meeting quota.</p>';
						if ($i==1) {
...
/*	
    ...
    public function RenderQuestion($intQuestionId) {
    	$objQuestion = Lemon50AssessmentQuestions::Load($intQuestionId);
    	return $objQuestion->Text;
    } 
 */
}

View50LemonAssessmentForm::Run('View50LemonAssessmentForm');
?>