...
class GroupAggregationResultForm extends InstituteForm {
	...
	protected function Form_Create() {
		...
		foreach($this->groupArray as $objGroupAssessmentId) {
			...
			$groupArray = Lemon50Assessment::LoadArrayByGroupId($objGroupAssessment->Id);	
			foreach($groupArray as $objGroup) {
				...
				if(!$objGroup->L) {
					...
					$resultArray = Lemon50AssessmentResults::LoadArrayByAssessmentId($objGroup->Id);
        ...
        $this->dtgUsers->MetaAddEditLinkColumn('<?=__SUBDIRECTORY__.InstituteForm::DirAssessments."/lemon/view50Assessment.php/".$_ITEM->Id ?>','<?= $_FORM->RenderName($_ITEM)?>','Name');		
	...
	public function chkSelected_Render(User $objUser, $intRow) {
		...
		$lemonArray = $objUser->GetLemon50AssessmentArray();
