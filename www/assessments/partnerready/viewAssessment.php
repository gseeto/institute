...
class ViewPartnerReadinessAssessmentForm extends InstituteForm {
	...
	protected function Form_Create() {
		...
		if($intUserId) { //show the assessment specified
			...
			$this->objPartneringReadinessAssessment = $assessmentArray[0];
			$objUser = User::Load($intUserId);
			$this->lblIntroduction->Text = 'Partnering Readiness Assessment for '.$objUser->FirstName. ' '.$objUser->LastName;
		} else { // show the user's assessment	
			$this->lblIntroduction->Text = 'Thank you for taking the Partnering Readiness Assessment.
Your results are provided below.';
		...
		for($i=0; $i<10;$i++) {
	 		$this->dtgAssessmentResultsArray[$i] = new PartneringReadinessResultsDataGrid($this);
			...
			$this->dtgAssessmentResultsArray[$i]->AddColumn(new QDataGridColumn('Willingness', '<?= $_ITEM->Willingness ?>','HtmlEntities=false'));
			$this->dtgAssessmentResultsArray[$i]->CellPadding = 5;
	
			$assessmentArray = PartneringReadinessResults::LoadArrayByAssessmentIdAndCategory($this->objPartneringReadinessAssessment->Id, $i+1);					
	...
	protected function initializeChart() {
		...
		foreach(CategoryType::$NameArray as $key=>$value) {			
			$resultArray = PartneringReadinessResults::LoadArrayByAssessmentIdAndCategory($this->objPartneringReadinessAssessment->Id,$key);
			$ptotal = $itotal = 0;
			foreach( $resultArray as $objResult) {
				$ptotal += $objResult->Importance;
				$itotal += $objResult->Willingness;
			}
			$objItem = new tenPArray();
			$objItem->P = $value;
			$objItem->importance = (count($resultArray))? $ptotal/count($resultArray) : 0;	
			$objItem->willingness = (count($resultArray)) ? $itotal/count($resultArray) :0;
			$associatedArray[] = $objItem;
			$delta[$value] = abs($objItem->importance - $objItem->willingness);
			$amount[$value] = $objItem->willingness;
		}
	...
	protected function btnGeneratePdf_Click() { 
		...
		$objPage1->drawText("Partnering Readiness Assessment Report", 50, $PageHeight, 'UTF-8');
		...
		$yPos -= $lineHeight;	
		$ZendImage = Zend_Pdf_Image::imageWithPath( __UPLOAD_DIR__.'/partnerready' . $this->objPartneringReadinessAssessment->Id. '.png');		
		...
		$objPage2->drawText("Large gaps between Importance and Willingness indicate areas for improvement. ", 10, $yPos, 'UTF-8');
		$yPos -= $lineHeight;
		...
		$objPage2->drawText("Your largest gap between Importance and Willingness appears to be under ".$this->lblDelta->Text .".This may indicate a critical area for the partnership to succeed.", 10, $yPos, 'UTF-8');
		$yPos -= $lineHeight;
		...
		for($i=0; $i<5;$i++) {
			...
			foreach($assessmentArray as $objQuestion){
				...
				$strStatement = PartneringReadinessQuestions::Load($objQuestion->QuestionId)->Text;
				...
				$objPage3->drawText($objQuestion->Willingness, $col4+12, $yPos, 'UTF-8');
			...
			$objPage3->drawText("Willingness", $col4+2, $yPosTableStart - $lineHeight, 'UTF-8');
		...
		for($i=5; $i<10;$i++) {
			...
			foreach($assessmentArray as $objQuestion){
				...
				$objPage4->drawText($objQuestion->Willingness, $col4+12, $yPos, 'UTF-8');
				if(strlen($strStatement)>100) {
					$yPos -= ($lineHeight+2)*2;
					$tableheight += ($lineHeight+2)*2;
				} else {
			...
			$objPage4->drawText("Willingness", $col4+2, $yPosTableStart - $lineHeight, 'UTF-8');
		...
		$pdfFile = '/partnerready' . $this->objPartneringReadinessAssessment->Id .rand(0,50). '.pdf';
...
ViewPartnerReadinessAssessmentForm::Run('ViewPartnerReadinessAssessmentForm');
class tenPArray {
			public $P;
			public $willingness;
			public $importance;
