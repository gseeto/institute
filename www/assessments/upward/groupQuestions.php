...
class UpwardGroupQuestions extends InstituteForm {
	...
	protected function Form_Create() {		
		...
		$this->arrayImportance = array();
	 	...
	 	for($i=0; $i<7;$i++) {
	 		$this->dtgAssessmentQuestionArray[$i] = new UpwardQuestionsDataGrid($this);
			...
			$assessmentArray = UpwardQuestions::LoadArrayByCategoryId($i+1);					
	...
	protected function btnSubmit_Click() {	
        for($i=0;$i<count($this->arrayPerformance); $i++) {
	        if($this->bEditing) {
	        	$objResults = UpwardResults::LoadResultByAssessmentIdAndQuestionId($this->objUpwardAssessment->Id, $i+1);
	        	if ($objResults == null) $objResults = new UpwardResults();
	        } else {
        		$objResults = new UpwardResults();
	        }
	        $objResults->AssessmentId = $this->objUpwardAssessment->Id;  
        ...
        if(!$this->bEditing) {
	        $this->objUpwardAssessment->ResourceStatusId = 2;
	        $this->objUpwardAssessment->Save();
        }
        $objGroupAssessment = GroupAssessmentList::Load($this->objUpwardAssessment->GroupId);
        if($objGroupAssessment) {
	        $objGroupAssessment->KeysLeft--;
	        $objGroupAssessment->Save();
        }
		QApplication::Redirect(__SUBDIRECTORY__.'/assessments/upward/groupResults.php');
	}
	
    public function lstImportance_Render(UpwardQuestions $objQuestions) {
        ...
        if (!$lstImportance) {
            ...
            if($this->bEditing) {
            	$value = UpwardResults::GetImportanceByAssessmentIdAndQuestionId($this->objUpwardAssessment->Id, $objQuestions->Id);
	            for ($i=1; $i<8; $i++) {
	            	if ($value == $i) 
	...
	public function lstPerformance_Render(UpwardQuestions $objQuestions) {
        // In order to keep track we will use explicitly defined control ids.
        ...
        if (!$lstPerformance) {
        	...
        	// Initialize values from previous assessment
            if($this->bEditing) {
            	$value = UpwardResults::GetPerformanceByAssessmentIdAndQuestionId($this->objUpwardAssessment->Id, $objQuestions->Id);
	            for ($i=1; $i<8; $i++) {
	            	if ($value == $i) 
...
UpwardGroupQuestions::Run('UpwardGroupQuestions');
?>