...
class SeasonalGroupInfoForm extends InstituteForm {
	...
	protected function btnLogin_Click() {
		...
		if ($objUser->IsResourceAssociated(Resource::Load(7))) {
			QApplication::Login($objLogin);
			$objAssessmentArray = IntegrationAssessment::LoadArrayByUserId($objLogin->Id);
			$objAssessment = $objAssessmentArray[0];
			$objAssessment->GroupId = $this->intGroupAssessment; 
			$objAssessment->Save();
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/seasons/newAssessment/edit');
		} else {
	     	$objAssessment = new IntegrationAssessment();
	     	$objAssessment->UserId = $objUser->Id;
	     	$objAssessment->ResourceId = 7; //LemonAssessment - going to have to find a nicer way of doing this
	     	$objAssessment->ResourceStatusId = 1; // initial state is untouched
	     	$objAssessment->GroupId = $this->intGroupAssessment;  	
	     	$objUser->AssociateResource(Resource::Load(6));
			$objAssessment->Save();
			QApplication::Login($objLogin);
			QApplication::Redirect(__SUBDIRECTORY__.'/assessments/seasons/newAssessment.php');
		}
	}
	
}

SeasonalGroupInfoForm::Run('SeasonalGroupInfoForm');
?>